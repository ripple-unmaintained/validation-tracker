...
import LedgerLogEntry from './ledger_log_entry'
    ...
    cmd.stdout.on("data", data => {
      ...
      for (let i=0; i<logEntries.length; i++) {
        ...
        if (logEntry.length) {
          ...
          } else if (logEntry.indexOf("NetworkOPs:NFO Consensus time for ledger") > 0) {
            let entry = new LedgerLogEntry(logEntry)
            this.onLedger(entry.toJSON())
          }
        }
      }
    })
